...
For more details about GenICam, GenTL, and GenApi, refer to the http://www.genicam.org website.
The GenICam GenApi standard document can be downloaded from http://www.emva.org/cms/upload/Standards/GenICam_Downloads/GenICam_Standard_v2_0.pdf.
The GenTL standard document can be downloaded from http://www.emva.org/cms/upload/Standards/GenICam_Downloads/GenICam_GenTL_1_5.pdf.
...
////Parametres de la cameraEXPORTED_TARGETS
////Parametres de l'acquisition
//bool disable_led; //desactivation des LEDs IR
//float acquisition_frame_rate; //fréquence d'acquisition (min-1)
//float exposure_time; //temps d'exposition (µs)
//int exposure_auto; //mode d'expo auto { off ; continue }
//float agility; //Vitesse de réaction de l'expo auto en mode continue [0-1]
//int delay; //Delais d'application du changement de taux d'expo (frame)

////Parametres du format d'image
////ROI (region of interrest)
//int width; //Largeur de l'image (pixel)
//int height; //hauteur de l'image (pixel)
//int depth; //profondeur (/)
//int offset_x; //decallage du 1er pixel depuis la gauche (pixel)
//int offset_y; //decallage du premier pixel depuis le haut (pixel)
//int offset_z; //decallage de profondeur (/)
////Composante de l'image
//bool range_enable; //activation de la composante profondeurEXPORTED_TARGETS
//int range_pixel_format; //format de la composante profondeur { image N&B ; image couleur ; nuage de points }
//bool intensity_enable; //activation de la composante intensite
//int intensity_pixel_format; //format de la composante intensite { image N&B }
//bool confidence_enable; //activation de la composante precision
//int confidence_pixel_format; //format de la composante precision { image N&B }

////Parametre de qualité d'image (filtrage)
//int confidence_threshold; //seuil sur la precision
//bool spatial_filter_enable; //activation du filtre spatial
//bool temporal_filter_enable; //activation du filtre temporel
//int temporal_filter_strenght; //duree d'integration du filtre temporel
//int outlier_tolerance; //distance max entre deux points (elimination des artefacts)

////Nom des parametres de la camera sur le rosparam serveur
//const string param_disable_led = "basler_tof/acqusition_control/disable_led";
//const string param_acquisition_frame_rate = "basler_tof/acqusition_control/acquisition_frame_rate";
//const string param_exposure_time = "basler_tof/acqusition_control/exposure_time";
//const string param_exposure_auto = "basler_tof/acqusition_control/exposure_auto";
//const string param_agility = "basler_tof/acqusition_control/agility";
//const string param_delay = "basler_tof/acqusition_control/delay";
//const string param_width = "basler_tof/image_format_control/width";
//const string param_height = "basler_tof/image_format_control/height";
//const string param_depth = "basler_tof/image_format_control/depth";
//const string param_offset_x = "basler_tof/image_format_control/offset_x";
//const string param_offset_y = "basler_tof/image_format_control/offset_y";
//const string param_offset_z = "basler_tof/image_format_control/offset_z";
//const string param_range_enable = "basler_tof/image_format_control/range_enable";
//const string param_range_pixel_format = "basler_tof/image_format_control/range_pixel_format";
//const string param_intensity_enable = "basler_tof/image_format_control/intensity_enable";
//const string param_intensity_pixel_format = "basler_tof/image_format_control/intensity_pixel_format";
//const string param_confidence_enable = "basler_tof/image_format_control/confidence_enable";
//const string param_confidence_pixel_format = "basler_tof/image_format_control/confidence_pixel_format";
//const string param_confidence_threshold = "basler_tof/image_quality_control/confidence_threshold";
//const string param_spatial_filter_enable = "basler_tof/image_quality_control/spatial_filter_enable";
//const string param_temporal_filter_enable = "basler_tof/image_quality_control/temporal_filter_enable";
//const string param_temporal_filter_strenght = "basler_tof/image_quality_control/temporal_filter_strenght";
//const string param_outlier_tolerance = "basler_tof/image_quality_control/outlier_tolerance";
...
///
/// \brief ErrorMessageDisplay : Fonction d'affichage des message d'erreur (en console)
...
///
/// \brief WarningMessageDisplayFonction d'affichage des message d'avertissement (en console)
/// \param message : message à afficher
///
...
///
/// \brief InfoMessageDisplayFonction d'affichage des message d'information (en console)
/// \param message : message à afficher
///
...
///
/// \brief OnNewImage : fonction traitant les nouvelles images
/// \param parts : buffer de la nouvelle image
///
void OnNewImage(BufferParts& parts)
{
    ...
    // si la profondeur est au format nuage de points
    if ( parts[0].dataFormat == PFNC_Coord3D_ABC32f )
    {
        ...
        PointCloud_msg.height = parts[0].height;
        PointCloud_msg.width = parts[0].width;
        PointCloud_msg.is_dense = false;
        PointCloud_msg.point_step = sizeof(float) * 3;
        PointCloud_msg.row_step = sizeof(float) * 3 * parts[0].width;
        ...
        //poiteur vers les points du nuage
        ...
        DepthImage_msg.height = parts[0].height;
        DepthImage_msg.width = parts[0].width;
        DepthImage_msg.encoding = sensor_msgs::image_encodings::RGB8;
        DepthImage_msg.step = parts[0].width * 3;
        ...
        for ( size_t i = 0; i < nPixel; ++i )
        {
            ...
            if(pPoint->IsValid())
            {
                // on colorise en fonction de la distance z et on convertie en RGB
                hsvcolor.h= ((pPoint->z/1000) / z_max) * 359;
                rgbcolor=hsv2rgb(hsvcolor);
                R=(uint8_t)(255*rgbcolor.r);
                G=(uint8_t)(255*rgbcolor.g);
                B=(uint8_t)(255*rgbcolor.b);
            }
            //sinon
            else
            {
            ...
            // et on ajoute dans les data du message image
            DepthImage_msg.data.push_back(R);
            DepthImage_msg.data.push_back(G);
            DepthImage_msg.data.push_back(B);
    ...
    // si la profondeur est au format RGB
    if ( parts[0].dataFormat == PFNC_RGB8 )
    {
        ...
        uint8_t *pDepth = (uint8_t*) parts[0].pData;
    ...
    // si la profondeur est au format N&B
    if ( parts[0].dataFormat == PFNC_Mono16 )
    {
        //on prepare le message
        sensor_msgs::Image DepthImage_msg;
        DepthImage_msg.header = header;
        ...
        DepthImage_msg.step = parts[0].width * 2;
    ...
    // si l'intensité est au format N&B (si elle est active)
    if ( parts[1].dataFormat == PFNC_Mono16 )
    {
        ...
        IntensityImage_msg.height = parts[1].height;
        IntensityImage_msg.width = parts[1].width;
        IntensityImage_msg.encoding = sensor_msgs::image_encodings::MONO16;
        IntensityImage_msg.step = parts[1].width * 2;

        //pointeur vers les pixel de l'image
        uint8_t *pIntensity = (uint8_t*) parts[1].pData;
    ...
    // si la precision est au format N&B (si elle est active)
    if ( parts[2].dataFormat == PFNC_Mono16 )
    {
        //on prepare le message
        sensor_msgs::Image ConfidenceImage_msg;
        ConfidenceImage_msg.header = header;
        ConfidenceImage_msg.height = parts[2].height;
        ConfidenceImage_msg.width = parts[2].width;
        ConfidenceImage_msg.encoding = sensor_msgs::image_encodings::MONO16;
        ConfidenceImage_msg.step = parts[2].width * 2;

        //pointeur vers les pixel de l'image
        uint8_t *pConfidence = (uint8_t*) parts[2].pData;
    ...
    //On affiche les infos sur la frame
    //InfoMessageDisplay("done : frame : " + std::to_string(seq_ID) + "(fps = " + std::to_string(FPS) + ")");
}

///
/// \brief CameraGrabbingThread : Fonction du thread permettant l'acquisition en continue
///
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        // Recupération des paramètres actuels de la caméra
        //        Camera.Get_Acquisition_Frame_Rate(&acquisition_frame_rate);
        //        Camera.Get_Agility(&agility);
        //        Camera.Get_Component_Confidence(&confidence_enable,&confidence_pixel_format);
        //        Camera.Get_Component_Intensity(&intensity_enable,&intensity_pixel_format);
        //        Camera.Get_Component_Range(&range_enable,&range_pixel_format);
        //        Camera.Get_Confidence_Threshold(&confidence_threshold);
        //        Camera.Get_Delay(&delay);
        //        Camera.Get_Depth(&depth);
        //        Camera.Get_Exposure_Auto(&exposure_auto);
        //        Camera.Get_Exposure_Time(&exposure_time);
        //        Camera.Get_Height(&height);
        //        Camera.Get_Led_Disable(&disable_led);
        //        Camera.Get_Offset_X(&offset_x);
        //        Camera.Get_Offset_Y(&offset_y);
        //        Camera.Get_Offset_Z(&offset_z);
        //        Camera.Get_Outlier_Tolerance(&outlier_tolerance);
        //        Camera.Get_Spatial_Filter_Enable(&spatial_filter_enable);
        //        Camera.Get_Temporal_Filter_Enable(&temporal_filter_enable);
        //        Camera.Get_Temporal_Filter_Strength(&temporal_filter_strenght);
        //        Camera.Get_Width(&width);

        //        // Ajout des paramètres caméra au RosParamServer
        //        ros::param::set(param_acquisition_frame_rate, acquisition_frame_rate);
        //        ros::param::set(param_agility, agility);
        //        ros::param::set(param_confidence_enable, confidence_enable);
        //        ros::param::set(param_confidence_pixel_format, confidence_pixel_format);
        //        ros::param::set(param_confidence_threshold, confidence_threshold);
        //        ros::param::set(param_delay, delay);
        //        ros::param::set(param_depth, depth);
        //        ros::param::set(param_disable_led, disable_led);
        //        ros::param::set(param_exposure_auto, exposure_auto);
        //        ros::param::set(param_exposure_time, exposure_time);
        //        ros::param::set(param_height, height);
        //        ros::param::set(param_intensity_enable, intensity_enable);
        //        ros::param::set(param_intensity_pixel_format, intensity_pixel_format);
        //        ros::param::set(param_offset_x, offset_x);
        //        ros::param::set(param_offset_y, offset_y);
        //        ros::param::set(param_offset_z, offset_z);
        //        ros::param::set(param_outlier_tolerance, outlier_tolerance);
        //        ros::param::set(param_range_enable, range_enable);
        //        ros::param::set(param_range_pixel_format, range_pixel_format);
        //        ros::param::set(param_spatial_filter_enable, spatial_filter_enable);
        //        ros::param::set(param_temporal_filter_enable, temporal_filter_enable);
        //        ros::param::set(param_temporal_filter_strenght, temporal_filter_strenght);
        //        ros::param::set(param_width, width);
        ...
        basler_tof::basler_tof_nodeConfig config;
        ...
        //        // Suppression des paramètres du RosParamServer
        //        ros::param::del(param_acquisition_frame_rate);
        //        ros::param::del(param_agility);
        //        ros::param::del(param_confidence_enable);
        //        ros::param::del(param_confidence_pixel_format);
        //        ros::param::del(param_confidence_threshold);
        //        ros::param::del(param_delay);
        //        ros::param::del(param_depth);
        //        ros::param::del(param_disable_led);
        //        ros::param::del(param_exposure_auto);
        //        ros::param::del(param_exposure_time);
        //        ros::param::del(param_height);
        //        ros::param::del(param_intensity_enable);
        //        ros::param::del(param_intensity_pixel_format);
        //        ros::param::del(param_offset_x);
        //        ros::param::del(param_offset_y);
        //        ros::param::del(param_offset_z);
        //        ros::param::del(param_outlier_tolerance);
        //        ros::param::del(param_range_enable);
        //        ros::param::del(param_range_pixel_format);
        //        ros::param::del(param_spatial_filter_enable);
        //        ros::param::del(param_temporal_filter_enable);
        //        ros::param::del(param_temporal_filter_strenght);
        //        ros::param::del(param_width);
